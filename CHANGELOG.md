...
## [v0.23.0] - 2026-05-14
### Agregado
- Asociación avanzada de scripts en el modal de alta de políticas: se puede vincular un
  recurso existente o crear uno nuevo desde el propio modal, con su propio panel de estilos.
- Gestión de recursos sincronizada en el editor de shared flows, resolviendo los errores de
  integración que dejaban el árbol y el editor desalineados.

### Corrección
- La lógica de inyección de flujos en `ProxyDetail` intercambiaba endpoints y sobrescribía el
  destino al insertar un flujo nuevo.

---

## Cómo se versiona este proyecto

El flujo de ramas es lineal y cada rama contiene a la anterior, así que los merges
son *fast-forward* y no generan commits de mezcla:

```
feat/<algo>  →  dev  →  qa  →  master
```

- **`master`** guarda lo que está funcionando. Cada avance se etiqueta con un tag
  `vX.Y.Z` sobre el commit que cierra ese hito.
- **`qa`** recibe lo que va a validarse. Si hace falta una versión candidata, se
  etiqueta `vX.Y.Z-rc`.
- **`dev`** integra las ramas de feature. Es donde se acumula el trabajo del día a día.
- **`feat/<algo>`** para cada funcionalidad, abierta desde `dev`.

### Numeración

Se sigue [SemVer](https://semver.org/lang/es/) sobre `0.x`, donde el proyecto aún no
promete estabilidad de interfaz:

- **Y** (menor) sube con cada hito funcional: una pantalla nueva, un módulo del backend,
  una integración.
- **Z** (parche) sube con correcciones que no añaden funcionalidad.
- **X** llegará a `1.0.0` cuando la consola cubra el ciclo completo de trabajo con el
  emulador y la interfaz deje de moverse.

### Al cerrar una versión

1. Mover lo que haya en `[Unreleased]` a una sección `## [vX.Y.Z] - AAAA-MM-DD`.
2. Actualizar `version` en `ui/package.json`.
3. Avanzar `dev → qa → master` y etiquetar el commit en `master`:
   `git tag -a vX.Y.Z -m "descripción" && git push origin vX.Y.Z`.

Los cambios pequeños también se anotan: una entrada de una línea en `[Unreleased]`
basta, y es lo que evita tener que reconstruir el historial leyendo diffs.
